...
import { UserPlus, FilePlus, Trash2, Users, FileText, LogOut, Upload, File, Pencil, UserCircle2, Database, Cloud, RefreshCcw, Sun, Moon, ShieldCheck, User, Search, Globe, Activity, Cpu, MessageSquare } from 'lucide-react';
...
const INTEGRATION_FIELDS = {
    ...
    cloudstorage: [
        ...
        { key: 'secret_key', label: 'Secret Key', placeholder: 'SECRET_KEY', isSecret: true }
    ]
};
...
const AdminDashboard = () => {
    ...
    const loadData = async () => {
        ...
        try {
            ...
            // Attempt to fetch analytics if route exists
            const aRes = await fetch(`${API_URL}/api/admin/analytics`).catch(() => null);

            const u = uRes.ok ? await uRes.json() : [];
            const d = dRes.ok ? await dRes.json() : [];
            const a = (aRes && aRes.ok) ? await aRes.json() : { total_users: u.length, total_docs: d.length, total_storage_mb: 0, status: {} };
        ...
        {/* AMBIENT BACKGROUND GLOWS */}
            ...
            <div onMouseEnter={() => setIsHovered(true)} onMouseLeave={() => setIsHovered(false)} className={`hidden md:flex bg-white/60 dark:bg-slate-900/60 backdrop-blur-xl text-slate-700 dark:text-slate-300 flex-col z-40 transition-all duration-300 ease-out border-r border-slate-200 dark:border-slate-800 shrink-0 ${isHovered ? 'w-64' : 'w-20'}`}>
                ...
                <div className="space-y-1.5 flex-1 px-3 overflow-hidden">
                    {/* Analytics is the top button now */}
                ...
                <div className="max-w-6xl mx-auto">
                    ...
                    <div className="flex items-center justify-between mb-8">
                        <h2 className="text-3xl md:text-4xl font-black text-slate-900 dark:text-white uppercase tracking-tight transition-colors">
                            {tab === 'staff' ? (t.accounts_mgmt || 'Accounts Management') : tab === 'docs' ? (t.docs_repo || 'Document Repository') : (t.sys_analytics || 'System Analytics & Health')}
                        </h2>
                        <div className="flex items-center gap-2">
                        ...
                        <div className="space-y-4 animate-in fade-in duration-500">
                            ...
                            <div className="flex items-center justify-between mb-2">
                                <div className="flex items-center gap-3 mt-1.5">
                                    <span className="w-4 h-4 rounded-full bg-emerald-500 shadow-[0_0_8px_rgba(16,185,129,0.8)] animate-pulse"></span>
                                    <p className="text-sm md:text-lg font-bold text-slate-500 dark:text-slate-400 uppercase tracking-widest">
                                        {formattedDate}
                                    </p>
                                    <span className="text-slate-300 dark:text-slate-700">|</span>
                                    <p className="text-xl md:text-2xl font-black text-amber-600 dark:text-amber-500 tracking-wider font-mono">
                                        {formattedTime}
                                    </p>
                                </div>
                                <button
                                ...
                                >
                                    <RefreshCcw size={14} className={isRefreshing ? "animate-spin" : ""} /> <span className="hidden sm:inline">{t.refresh_stats || 'Refresh'}</span>
                                </button>
                            </div>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 pt-4">
                                <div className={`${cardStyle} p-6 flex flex-col items-center justify-center text-center`}>
                                    ...
                                    <h4 className="text-4xl font-black dark:text-white mb-1">{analytics.total_users || data.users.length}</h4>
                                ...
                                <div className={`${cardStyle} p-6 flex flex-col items-center justify-center text-center`}>
                                    ...
                                    <h4 className="text-4xl font-black dark:text-white mb-1">{analytics.total_docs || data.docs.length}</h4>
                                ...
                                <form onSubmit={handleAddUser} className="grid grid-cols-1 sm:grid-cols-12 gap-4 items-end relative z-10">
                                    ...
                                    <div className="sm:col-span-3">
                                        <button className={primaryBtnStyle}>
                                            <UserPlus size={16}/> {t.generate_acc}
                                        </button>
                                    </div>

                                    <div className="sm:col-span-12 text-[10px] text-slate-500 dark:text-slate-400 italic ml-1 mt-1 font-medium">
                                        {t.acc_notice}
                                    </div>
                                </form>
                            </section>
                                    ...
                                    {data.docs.map(d => (
                                        <div key={d.id} className="p-4 border-b border-slate-200/50 dark:border-white/5 flex justify-between items-center hover:bg-white/50 dark:hover:bg-white/5 transition-colors">
                                            <div className="flex items-center gap-3 overflow-hidden">
                                                ...
                                                <div className="overflow-hidden">
                                                    <p className="font-bold truncate text-slate-800 dark:text-white tracking-tight">{d.title}</p>
                                                    <p className="text-[10px] font-bold text-slate-500 uppercase tracking-widest mt-0.5">{d.category} • {d.file_type}</p>
                                                </div>
                                            </div>
            ...
            <div className="md:hidden fixed bottom-0 left-0 right-0 bg-white/80 dark:bg-[#0a0a0a]/80 backdrop-blur-2xl saturate-150 border-t border-slate-200 dark:border-white/5 flex justify-around items-center z-40 pb-safe shadow-[0_-8px_30px_rgba(0,0,0,0.1)] transition-colors h-16">
                {[ ['analytics', Activity, t.tab_analytics || 'Health'], ['staff', Users, t.tab_accounts || 'Users'], ['docs', FileText, t.tab_docs || 'Files'] ].map(([id, Icon, label]) => (
