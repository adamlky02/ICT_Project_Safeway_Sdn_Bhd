...
import { UserPlus, FilePlus, Trash2, Users, FileText, LogOut, Upload, File, Pencil, UserCircle2, RefreshCcw, Sun, Moon, ShieldCheck, User, Search, Globe, Activity, Database, Cloud, Cpu, MessageSquare, ChevronDown, Check } from 'lucide-react';
...
const AdminDashboard = () => {
    ...
    // --- DARK MODE LOGIC (SessionStorage ensures default is Light Mode) ---
    const [isDarkMode, setIsDarkMode] = useState(() => sessionStorage.getItem('theme') === 'dark');

    useEffect(() => {
        const root = document.documentElement;
        if (isDarkMode) { root.classList.add('dark'); sessionStorage.setItem('theme', 'dark'); }
        else { root.classList.remove('dark'); sessionStorage.setItem('theme', 'light'); }
    }, [isDarkMode]);
    ...
    const loadProfileData = async () => {
        try {
            ...
            if (!userDataStr) { navigate('/login'); return; }
            const userData = JSON.parse(userDataStr);
            const res = await fetch(`${API_URL}/api/profile/${userData.id}`);
    ...
    const loadData = async () => {
        ...
        try {
            ...
            const aRes = await fetch(`${API_URL}/api/admin/analytics`);
        ...
        <div className="flex h-screen bg-slate-50 dark:bg-[#0a0a0a] transition-colors duration-500 overflow-hidden relative font-sans">
            ...
            <div className="md:hidden bg-white/80 dark:bg-[#0a0a0a]/80 backdrop-blur-md border-b border-slate-200 dark:border-white/5 p-4 flex justify-between items-center fixed top-0 left-0 right-0 z-20 transition-colors">
                ...
                <div className="flex items-center gap-2">
                    ...
                    <button onClick={() => {setIsDarkMode(!isDarkMode); window.dispatchEvent(new Event('themeChanged'));}} className="p-2 rounded-full bg-slate-100 dark:bg-white/5 text-slate-600 dark:text-slate-300 transition-colors">
            ...
            <div onMouseEnter={() => setIsHovered(true)} onMouseLeave={() => setIsHovered(false)} className={`hidden md:flex bg-white/60 dark:bg-slate-900/60 backdrop-blur-xl text-slate-700 dark:text-slate-300 flex-col z-40 transition-all duration-300 ease-out border-r border-slate-200 dark:border-slate-800 shrink-0 ${isHovered ? 'w-64' : 'w-20'}`}>
                ...
                <div className="mt-auto mb-6 px-3 space-y-1.5 pt-4 border-t border-slate-300/50 dark:border-white/10">
                    ...
                    <button onClick={() => {setIsDarkMode(!isDarkMode); window.dispatchEvent(new Event('themeChanged'));}} className={`w-full flex items-center py-3 rounded-2xl hover:bg-slate-200 dark:hover:bg-slate-800 transition-all font-medium text-slate-600 dark:text-slate-400 ${isHovered ? 'px-4' : 'justify-center'}`}>
            ...
            <div className="flex-1 p-4 pt-24 pb-24 md:p-10 overflow-y-auto w-full transition-colors duration-300 relative z-10 custom-scrollbar">
                <div className="max-w-6xl mx-auto">
                    ...
                    {/* HEADER WITH CHAT & UNIFIED PROFILE DROPDOWN */}
                    <div className="flex items-center justify-between mb-8">
                        <h2 className="text-3xl md:text-4xl font-black text-slate-900 dark:text-white uppercase tracking-tight transition-colors">
                            {tab === 'staff' ? t.accounts_mgmt || 'Accounts' : tab === 'docs' ? t.docs_repo || 'Documents' : t.sys_analytics || 'System Analytics'}
                        </h2>
                        ...
                        <div className="flex items-center gap-2">
                            ...
                            <div className="relative" ref={profileBtnRef}>
                                <button
                                    ...
                                    className="flex items-center gap-2 px-3 py-2 rounded-2xl bg-white/40 dark:bg-white/10 backdrop-blur-xl border border-white/60 dark:border-white/20 text-slate-800 dark:text-white hover:bg-white/60 dark:hover:bg-white/20 transition-all shadow-sm font-bold text-sm"
                                >
                                ...
                                {dropdownOpen && (
                                    <div className="absolute right-0 mt-3 w-64 bg-white/95 dark:bg-[#0a0a0a]/95 backdrop-blur-3xl saturate-150 border border-slate-200 dark:border-white/10 rounded-[2rem] shadow-2xl z-50 p-6 flex flex-col items-center animate-in fade-in slide-in-from-top-2 duration-300">
                                        ...
                                        <div className="text-lg font-black text-slate-800 dark:text-white mt-2 tracking-tight">
                                            {profile?.full_name || 'Safeway Admin'}
                                        </div>
                        ...
                        <div className="space-y-4 animate-in fade-in duration-500">
                            <div>
                                <div className="flex items-center gap-3 mt-1.5">
                                    <span className="w-3 h-3 rounded-full bg-emerald-500 shadow-[0_0_8px_rgba(16,185,129,0.8)] animate-pulse"></span>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 pt-4">
                                <div className={`${cardStyle} p-6 flex flex-col items-center justify-center text-center`}>
                                    ...
                                    <h4 className="text-4xl font-black dark:text-white mb-1">{analytics.total_users}</h4>
                                ...
                                <div className={`${cardStyle} p-6 flex flex-col items-center justify-center text-center`}>
                                    ...
                                    <h4 className="text-4xl font-black dark:text-white mb-1">{analytics.total_docs}</h4>
                                ...
                                <div className={`${cardStyle} p-6 flex flex-col items-center justify-center text-center`}>
                                    ...
                                    <h4 className="text-4xl font-black dark:text-white mb-1">{analytics.total_storage_mb} <span className="text-lg text-slate-400">MB</span></h4>
                            ...
                            <div className={`${cardStyle} p-6 md:p-8 space-y-6 mt-4`}>
                                <div className="flex justify-between items-center border-b border-slate-200 dark:border-white/10 pb-3">
                                    <h4 className="text-sm font-black text-slate-400 uppercase tracking-widest">{t.system_status || 'Live Status'}</h4>
                                    <button onClick={loadData} disabled={isRefreshing} className="flex items-center gap-2 text-[10px] font-bold uppercase tracking-widest text-blue-600 dark:text-amber-500 hover:text-blue-800 dark:hover:text-amber-400 transition-colors">
                                        <RefreshCcw size={12} className={isRefreshing ? "animate-spin" : ""} /> {t.refresh_stats || 'Refresh'}
                                    </button>
                                </div>
                            ...
                            <div className="flex flex-col sm:flex-row gap-4 items-end justify-between">
                                <div className="space-y-1.5 w-full sm:w-72">
                                    ...
                                    <div className="relative group">
                                        ...
                                        <input type="text" placeholder={t.search_placeholder || 'Search...'} className={`${inputStyle} pl-9`} value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} />
                                    </div>
                                </div>
                            ...
                            <section className={`${cardStyle} p-5`}>
                                ...
                                <form onSubmit={handleAddUser} className="grid grid-cols-1 sm:grid-cols-12 gap-4 items-end relative z-10">
                                    <div className="sm:col-span-5 flex flex-col gap-1.5">
                                        <label className="text-[11px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wide ml-1">{t.emp_name || 'Employee Name'}</label>
                                    ...
                                    <div className="sm:col-span-4 flex flex-col gap-1.5">
                                        <label className="text-[11px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wide ml-1">{t.email || 'Email'} <span className="text-[9px] lowercase opacity-70">{t.email_prefix || '(prefix)'}</span></label>
                                    ...
                                    <div className="sm:col-span-3">
                                        <button className={primaryBtnStyle}><UserPlus size={16}/> {t.generate_acc || 'Generate Account'}</button>
                                    </div>
                                </form>
                            </section>
                            ...
                            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                                <div className={`${cardStyle} flex flex-col h-[380px]`}>
                                    ...
                                    <div className="bg-gradient-to-r from-amber-500 to-orange-500 p-3.5 flex justify-between items-center text-slate-900 shrink-0 font-black text-xs uppercase tracking-widest relative z-10 border-b border-white/20">
                                        <div className="flex items-center gap-2"><ShieldCheck size={14}/> {t.admins || 'Administrators'}</div>
                                        <span className="bg-slate-900/10 px-2.5 py-0.5 rounded-full">{adminUsers.length}</span>
                                    </div>
                                    <div className="flex-1 overflow-y-auto custom-scrollbar relative z-10">
                                        {adminUsers.length === 0 ? <div className="p-10 text-center text-slate-400 italic text-sm">{t.no_matches || 'No matches found.'}</div> : adminUsers.map(u => <UserRow key={u.id} user={u} isStaffTable={false} />)}
                                    </div>
                                </div>
                                ...
                                <div className={`${cardStyle} flex flex-col h-[380px]`}>
                                    ...
                                    <div className="bg-slate-200/50 dark:bg-slate-800/80 p-3.5 flex justify-between items-center text-slate-800 dark:text-white shrink-0 font-black text-xs uppercase tracking-widest border-b border-slate-300 dark:border-slate-700 relative z-10">
                                        <div className="flex items-center gap-2"><User size={14}/> {t.internal_staff || 'Internal Staff'}</div>
                                        <span className="bg-white/40 dark:bg-white/10 px-2.5 py-0.5 rounded-full">{staffUsers.length}</span>
                                    </div>
                                    <div className="flex-1 overflow-y-auto custom-scrollbar relative z-10">
                                        {staffUsers.length === 0 ? <div className="p-10 text-center text-slate-400 italic text-sm">{t.no_matches || 'No matches found.'}</div> : staffUsers.map(u => <UserRow key={u.id} user={u} isStaffTable={true} />)}
                                    </div>
                                </div>
                            </div>
                        </div>
                    ...
                    {tab === 'docs' && (
                        <div className="space-y-6 animate-in fade-in duration-500">
                            <form onSubmit={handleFileUpload} className={`${cardStyle} p-6 md:p-8 space-y-6`}>
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 relative z-10">
                                    <div className="space-y-1.5">
                                        <label className="text-[10px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-widest ml-1">{t.doc_title || 'Document Title'}</label>
                                    ...
                                    <div className="space-y-1.5">
                                        <label className="text-[10px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-widest ml-1">{t.category || 'Category'}</label>
                                ...
                                <div className="border border-dashed border-slate-300 dark:border-white/20 p-10 rounded-3xl flex flex-col items-center justify-center bg-white/40 dark:bg-black/20 hover:border-amber-400 dark:hover:border-amber-500/50 transition-all cursor-pointer relative text-center shadow-inner z-10">
                                    ...
                                    <p className="text-slate-800 dark:text-slate-200 text-sm font-bold tracking-wide">{selectedFile ? selectedFile.name : t.choose_file || 'Tap or Drag file to upload'}</p>
                                    <p className="text-[10px] text-slate-500 dark:text-slate-400 mt-2 font-medium">{t.auto_embed_notice || 'Files are automatically embedded for AI Search.'}</p>
                                </div>
                                <div className="relative z-10">
                                    <button className={primaryBtnStyle}><Cloud size={18}/> {t.upload_repo || 'Upload to Repository'}</button>
                                </div>
                            </form>
                            ...
                            <div className={`${cardStyle} overflow-hidden`}>
                                <div className="absolute inset-0 rounded-[2rem] shadow-[inset_0_1px_1px_rgba(255,255,255,0.6)] dark:shadow-[inset_0_1px_1px_rgba(255,255,255,0.1)] pointer-events-none"></div>
                                <div className="relative z-10">
                                    <div className="bg-slate-50 dark:bg-white/5 p-4 border-b border-slate-200 dark:border-white/5 text-xs font-bold text-slate-700 dark:text-slate-300 uppercase tracking-widest">{t.stored_docs || 'Stored Documents'}</div>
                                    {data.docs.map(d => (
                                    ...
                                    {data.docs.length === 0 && <p className="p-8 text-center text-slate-500 italic text-sm">{t.no_matches || 'No documents found.'}</p>}
                                </div>
                            </div>
                        </div>
                    )}
            ...
            <div className="md:hidden fixed bottom-0 left-0 right-0 bg-white/80 dark:bg-[#0a0a0a]/80 backdrop-blur-2xl saturate-150 border-t border-slate-200 dark:border-white/5 flex justify-around items-center z-40 pb-safe shadow-[0_-8px_30px_rgba(0,0,0,0.1)] transition-colors h-16">
                {[ ['analytics', Activity, t.tab_analytics || 'Health'], ['staff', Users, t.tab_accounts || 'Accounts'], ['docs', FileText, t.tab_docs || 'Files'] ].map(([id, Icon, label]) => (
            ...
            {editingUser && (
                <div className="fixed inset-0 bg-slate-900/60 dark:bg-black/60 backdrop-blur-sm flex items-center justify-center p-4 z-50 transition-all">
                    <div className="bg-white/90 dark:bg-[#0a0a0a]/90 backdrop-blur-2xl saturate-150 w-full max-w-xl rounded-[2.5rem] shadow-2xl border border-white/50 dark:border-white/10 p-8 max-h-[90vh] overflow-y-auto relative">
                        ...
                        <h3 className="text-2xl font-black text-slate-900 dark:text-white mb-6 uppercase tracking-tight relative z-10">{t.modify_acc || 'Modify Account'}</h3>
                        <form onSubmit={handleEditUser} className="space-y-6 relative z-10">
                            <div className="space-y-2">
                                <h4 className="text-[10px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-widest ml-1">{t.emp_name || 'Employee Name'}</h4>
                                <div className="grid grid-cols-2 gap-4">
                                    <input className={inputStyle} value={editForm.first_name} onChange={e => setEditForm({ ...editForm, first_name: e.target.value })} required placeholder={t.first_name || 'First Name'} />
                                    <input className={inputStyle} value={editForm.last_name} onChange={e => setEditForm({ ...editForm, last_name: e.target.value })} required placeholder={t.last_name || 'Last Name'} />
                                </div>
                            </div>
                            <div className="space-y-2">
                                <h4 className="text-[10px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-widest ml-1">{t.email || 'Email'}</h4>
                            ...
                            <div className="space-y-2">
                                ...
                                <input type="password" placeholder={t.new_pass_placeholder || 'New password (leave blank to keep)'} className={inputStyle} value={editForm.password} onChange={e => setEditForm({ ...editForm, password: e.target.value })} />
                            </div>
                            <div className="flex items-center gap-4 bg-slate-50/50 dark:bg-black/20 p-4 rounded-xl border border-slate-200 dark:border-white/10 shadow-inner backdrop-blur-sm">
                                ...
                                <div><label htmlFor="promote" className="font-bold text-sm dark:text-white">{t.admin_privileges || 'Admin Privileges'}</label></div>
                            </div>
                            <div className="flex justify-end gap-3 pt-4 border-t border-slate-200/50 dark:border-white/5">
                                <button type="button" onClick={() => setEditingUser(null)} className="px-6 py-2.5 rounded-xl text-sm font-bold text-slate-600 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-white/5 transition-colors">{t.cancel || 'Cancel'}</button>
                                <button type="submit" className={primaryBtnStyle + " w-auto px-8 py-2.5"}>{t.save_changes || 'Save Changes'}</button>
                            </div>
                        </form>
                    </div>
                </div>
            ...
            {showPasswordModal && (
                <div className="fixed inset-0 bg-slate-900/80 dark:bg-black/80 backdrop-blur-md flex items-center justify-center z-50 p-4 transition-all">
                    <div className="bg-white/90 dark:bg-[#0a0a0a]/90 backdrop-blur-2xl saturate-150 rounded-[2.5rem] shadow-2xl p-8 max-w-md w-full text-center border border-white/50 dark:border-white/10 relative">
                        <div className="absolute inset-0 rounded-[2.5rem] shadow-[inset_0_1px_1px_rgba(255,255,255,0.8)] dark:shadow-[inset_0_1px_1px_rgba(255,255,255,0.15)] pointer-events-none"></div>
                        <div className="w-20 h-20 bg-emerald-50 dark:bg-emerald-500/10 text-emerald-600 dark:text-emerald-500 mx-auto rounded-full flex items-center justify-center mb-6 border border-emerald-200 dark:border-emerald-500/20 shadow-inner relative z-10"><Check size={40}/></div>
                        <h3 className="text-2xl font-black dark:text-white mb-2 uppercase tracking-tight relative z-10">{t.acc_ready || 'Account Ready'}</h3>
                        <div className="bg-slate-50/50 dark:bg-black/20 p-5 rounded-2xl border border-slate-200 dark:border-white/10 mb-6 space-y-3 shadow-inner backdrop-blur-sm relative z-10">
                            <div className="text-left"><p className="text-[10px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-widest">{t.email || 'Email'}</p><p className="font-mono font-bold text-slate-800 dark:text-white text-sm">{generatedPassword.email}</p></div>
                        ...
                        <div className="flex gap-3 relative z-10">
                            <button onClick={() => { navigator.clipboard.writeText(`Safeway Access\nEmail: ${generatedPassword.email}\nPass: ${generatedPassword.password}`); alert(t.copy + '!'); }} className="flex-1 bg-slate-100 dark:bg-white/5 p-3 rounded-xl font-bold text-sm text-slate-700 dark:text-white hover:bg-slate-200 dark:hover:bg-white/10 transition-colors">{t.copy || 'Copy'}</button>
                            <button onClick={() => setShowPasswordModal(false)} className="flex-1 bg-slate-900 dark:bg-white text-white dark:text-slate-900 p-3 rounded-xl font-bold text-sm transition-colors hover:scale-[0.98]">{t.done || 'Done'}</button>
                        </div>
                    </div>
                </div>
            )}
            ...
            {showPromoteConfirm && (
                <div className="fixed inset-0 bg-slate-900/60 dark:bg-black/60 backdrop-blur-md flex items-center justify-center p-4 z-50 transition-all">
                    <div className="bg-white/90 dark:bg-[#0a0a0a]/90 backdrop-blur-2xl saturate-150 p-8 rounded-[2.5rem] shadow-2xl max-w-sm w-full text-center border border-white/50 dark:border-white/10 relative">
                        <div className="absolute inset-0 rounded-[2.5rem] shadow-[inset_0_1px_1px_rgba(255,255,255,0.8)] dark:shadow-[inset_0_1px_1px_rgba(255,255,255,0.15)] pointer-events-none"></div>
                        <h3 className="text-xl font-black text-slate-900 dark:text-white mb-3 uppercase tracking-tight relative z-10">{t.change_role || 'Security Change'}</h3>
                        <p className="text-sm text-slate-600 dark:text-slate-400 mb-8 font-medium relative z-10">{t.switch_access || 'Switch access level to'} <span className="font-bold text-amber-600 dark:text-amber-500 uppercase">{pendingRole}</span>?</p>
                        <div className="grid grid-cols-2 gap-3 relative z-10">
                            <button onClick={() => handlePromoteChoice(false)} className="p-3 rounded-xl border border-slate-200 dark:border-white/10 font-bold text-slate-600 dark:text-slate-300 text-sm hover:bg-slate-50 dark:hover:bg-white/5 transition-colors">{t.cancel || 'Cancel'}</button>
                            <button onClick={() => handlePromoteChoice(true)} className={primaryBtnStyle + " p-3"}>{t.confirm || 'Confirm'}</button>
                        </div>
                    </div>
                </div>
            )}
